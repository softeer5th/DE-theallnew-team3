...
def collect_target_videos(input_date, car_name):
    ...
    # 검색 기간 설정
    published_after = f"{input_date}-01T00:00:00Z"  # 해당 월 1일 시작
    published_before = f"{get_next_month(input_date)}-01T00:00:00Z"  # 해당 월 말일 종료
    ...
    with open(f"data/youtube_{input_date}_{car_name}.csv", "w") as f:
        for video_id in video_ids:
            f.write(video_id + "\n")


def process_videos(input_date, car_name):
    with open(f"data/youtube_{input_date}_{car_name}.csv", "r") as f:
    ...
    json_filename = f"data/youtube_{input_date}_{car_name}.json"
    with open(json_filename, "w", encoding="utf-8") as jsonfile:
        json.dump(video_data_list, jsonfile, ensure_ascii=False, indent=4)


def extract_youtube(input_date, car_name):
    print("Youtube - collect_target_videos")
    collect_target_videos(input_date, car_name)
    print("Youtube - process_videos")
    process_videos(input_date, car_name)
