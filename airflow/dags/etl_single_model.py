...
with DAG(
    ...
    get_target_files = S3ListOperator(
        ...
        bucket=BUCKET_NAME,
        prefix="{{ params.car_type }}/{{ macros.ds_format(ds, '%Y-%m-%d', '%Y') }}/{{ macros.ds_format(ds, '%Y-%m-%d', '%m') }}/{{ macros.ds_format(ds, '%Y-%m-%d', '%d') }}/sentence_data/",
    )
    ...
    for copy_param in COPY_PARAMS:
        s3_key = (
            "{{ params.car_type }}/{{ macros.ds_format(ds, '%Y-%m-%d', '%Y') }}/{{ macros.ds_format(ds, '%Y-%m-%d', '%m') }}/{{ macros.ds_format(ds, '%Y-%m-%d', '%d') }}/"
            + copy_param["S3_KEY"]
        )
